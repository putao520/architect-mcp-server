...
async function runWorkerTask(task, baseCwd, env, mode = 'fast') {
  ...
  // steps 链：多个子任务串行执行，共享上下文
  let description = task.description;
  if (task.steps && task.steps.length > 0) {
    description = task.steps.map((step, i) => `## Step ${i + 1}: ${step}`).join('\n\n');
  }
...
任务链（steps）：共享上下文的多个子任务放入同一个 task 的 steps 数组，一个 worker 串行执行，避免重复冷启动。无依赖的 task 之间并行。`,
    {
      ...
      })).describe('要并行执行的任务列表。每个 task 独立 worker，task 内 steps 串行共享上下文'),
